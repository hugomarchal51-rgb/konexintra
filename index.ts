// Konekt AI Operator edge function — full voice assistant
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // Fetch all user data
    const [docsResult, revenueResult, profileResult, emailsResult, tasksResult] = await Promise.all([
    ...
    const wantsCreate = lowerMsg.includes("cré") || lowerMsg.includes("fait") || lowerMsg.includes("nouveau") || lowerMsg.includes("ajoute") || lowerMsg.includes("génére");
    ...
    if ((wantsDevis || wantsFacture) && wantsCreate) {
      ...
      if (missing.length === 0) {
        // All info provided — create the document
    ...
    // === AIDE ===
      ...
      response = `Je peux vous aider avec : la création de devis et factures, le suivi de votre chiffre d'affaires, la gestion de vos emails, la création de tâches et rappels, les relances clients, et la préparation de vos rendez-vous. Dites-moi simplement ce dont vous avez besoin !`;
    }
    // === FALLBACK ===
    else {
      response = `J'ai bien noté votre demande : « ${userMessage} ». Je peux créer un document, ajouter un revenu, vérifier vos emails, ou créer une tâche. Que souhaitez-vous que je fasse exactement ?`;
    }
